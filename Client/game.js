...
function showMultiplayerRanks(boardSize){
	...
	xhr.onreadystatechange = function() {
		if(this.readyState < 4)
			...
			document.getElementById("tableRankingDiv").innerHTML = "Error! Invalid number";
	}
		...
		xhr.onreadystatechange = function() {
			if(this.readyState < 4)
				...
				if(json["error"]=="Invalid size")
					messageH.innerHTML = "Error! The board size can only be positive whole numbers.";
				else
					messageH.innerHTML = "Error! You can't play against yourself.";
			}
			else if(this.status >= 500)
				messageH.innerHTML = "Error! Couldn't connect to the server.";
		}
...
function nimGame(dif, firstPlayer, boardSize){
	...
	this.updateMessageDiv = function(){
		if((this.moves%2==0 && this.firstPlayer=="player") || (this.moves%2!=0 && this.firstPlayer!="player")){
			document.getElementById("messageH1").innerHTML = "-> It's <ins>" + username + "</ins> turn <-";
			setTimer();
		}
		else{
...
function nimOnlineGame(gameId, boardSize){
	...
	this.updateMessageDiv = function(){
		clearTimeout(timeOutMessage);
		document.getElementById("messageH1").innerHTML = "-> It's <ins>" + this.turn + "</ins> turn <-";
	}
		...
		xhr.onreadystatechange = function() {
			if(this.readyState < 4)
				...
				if(json["error"] == "Not your turn to play"){
					...
					document.getElementById("messageH1").innerHTML = "Error! The stack cannot have a negative number of pieces.";
					timeOutMessage = setTimeout("mainGame.updateMessageDiv()", 3000);
				}
			}
		}
...
function Piece(x, y){
	...
	this.html.src = "Images/piece.png";
